    ...
    try {
        ...
        for (const assetConfig of endpointsToFetch) {
            try {
                ...
                console.log(`[Location Exporter] ${assetConfig.key} response structure:`, {
                    responseKeys,
                    dataKeys,
                    lookingFor: assetConfig.dataKey,
                    hasDirectKey: !!(response && response[assetConfig.dataKey]),
                    hasDataKey: !!(response?.data && response.data[assetConfig.dataKey])
                });
                ...
                if (response) {
                    ...
                    if (!assets) {
                        assets = [];
                        console.warn(`[Location Exporter] Could not find data for ${assetConfig.key}. Full response:`, JSON.stringify(response).substring(0, 1000));
                    }
                    ...
                    console.log(`[Location Exporter] Fetched ${assets.length} ${assetConfig.key} from ${foundAt}`);
                } else {
                    locationData[assetConfig.key] = [];
                    console.log(`[Location Exporter] No response for ${assetConfig.key}`);
                }
            } catch (error) {
        ...
        const allAssetKeys = [
            ...
            'voice_ai_agents', 'ai_employees', 'documents', 'snippets'
        ];
    ...
    for (const assetType of assetsToExport) {
        ...
        if (assets && assets.length > 0) {
            ...
            if (assetType.key === 'workflow') {
                ...
                console.log('[Location Exporter] ✅ WORKFLOW TRIGGERS FETCH TRIGGERED');
                ...
                if (workflowTriggers.length > 0) {
                    ...
                    sheetsCreated++;
                    console.log(`[Location Exporter] Created Workflow_Triggers sheet: ${workflowTriggers.length} triggers`);
                } else {
                    console.log('[Location Exporter] No workflow triggers found');
                }
            }
            ...
            // Special handling for Custom Fields - fetch ALL custom fields including opportunity and custom object fields
                ...
                console.log('[Location Exporter] ✅ CUSTOM FIELD ENRICHMENT TRIGGERED');
                ...
                // Update summary and master list with actual count
                const summaryIdx = summaryData.findIndex(row => row[0] === 'Custom Fields');
                if (summaryIdx !== -1) {
                    summaryData[summaryIdx][1] = allCustomFields.length;
                }

                // Add all custom fields to master list
                allCustomFields.forEach(field => {
                    const id = field._id || field.id || '';
                    const name = field.name || '';
                    // Check if already in master list
                    const exists = masterListData.some(row => row[0] === id);
                    if (!exists) {
                        masterListData.push([id, name, 'Custom Fields']);
                    }
                });

                totalAssets = totalAssets - assets.length + allCustomFields.length;

                XLSX.utils.book_append_sheet(workbook, worksheet, 'Custom_Fields');
                sheetsCreated++;
                console.log(`[Location Exporter] Created enriched sheet for Custom Fields: ${allCustomFields.length} items (including all models)`);
            }
            // Special handling for Voice AI Agents - enrich with full configuration
            else if (assetType.key === 'voice_ai_agents') {
                console.log('[Location Exporter] ✅ VOICE AI AGENTS ENRICHMENT TRIGGERED');
                ...
                sheetsCreated++;
                console.log(`[Location Exporter] Created enriched sheet for Voice AI Agents: ${enrichedAgents.length} items`);
            }
            // Special handling for AI Employees - enrich with full configuration
            else if (assetType.key === 'ai_employees') {
                console.log('[Location Exporter] ✅ AI EMPLOYEES ENRICHMENT TRIGGERED');
                ...
                sheetsCreated++;
                console.log(`[Location Exporter] Created enriched sheet for AI Employees: ${enrichedEmployees.length} items`);
            }
            // Special handling for Knowledge Bases - enrich with files, URLs, FAQs
            else if (assetType.key === 'knowledge_bases') {
                console.log('[Location Exporter] ✅ KNOWLEDGE BASE ENRICHMENT TRIGGERED');
                ...
                sheetsCreated++;
                console.log(`[Location Exporter] Created enriched sheet for Knowledge Bases: ${enrichedKBs.length} items`);
            }
            // Special handling for Documents - enrich with template details
            else if (assetType.key === 'documents') {
                console.log('[Location Exporter] ✅ DOCUMENTS ENRICHMENT TRIGGERED');
                ...
                sheetsCreated++;
                console.log(`[Location Exporter] Created enriched sheet for Documents: ${enrichedDocs.length} items`);
            }
            // Special handling for Snippets - enrich with template details
            else if (assetType.key === 'snippets') {
                console.log('[Location Exporter] ✅ SNIPPETS ENRICHMENT TRIGGERED');
                ...
                sheetsCreated++;
                console.log(`[Location Exporter] Created enriched sheet for Snippets: ${enrichedSnippets.length} items`);
            }
...
async function convertLocationToCSVs(locationData, locationId, selectedAssets = null) {
    ...
    for (const assetType of assetsToExport) {
        ...
        if (assetType.key === 'custom_fields' && locationId) {
            console.log('[Location Exporter CSV] Fetching all custom fields (contacts, opportunities, custom objects)...');
            assets = await fetchAllCustomFieldsForLocation(locationId);
            console.log(`[Location Exporter CSV] Fetched ${assets.length} total custom fields`);
        }
    ...
    for (const assetType of assetsToExport) {
        ...
        if (assets && assets.length > 0) {
            ...
            if (assetType.key === 'workflow' && locationId) {
                ...
                // Fetch and create Workflow Triggers sheet
                console.log('[Snapshot Exporter] ✅ WORKFLOW TRIGGERS FETCH TRIGGERED');
                ...
                if (workflowTriggers.length > 0) {
                    ...
                    XLSX.utils.book_append_sheet(workbook, triggerWorksheet, 'Workflow_Triggers');
                    sheetsCreated++;
                    console.log(`[Snapshot Exporter] Created Workflow_Triggers sheet: ${filteredTriggers.length} triggers`);
                } else {
                    console.log('[Snapshot Exporter] No workflow triggers found');
                }
            }
            // Special handling for Forms - enrich with full data
            ...
            // Special handling for Voice AI Agents - enrich with full configuration
                ...
                console.log('[Snapshot Exporter] ✅ VOICE AI AGENTS ENRICHMENT TRIGGERED');
                ...
                XLSX.utils.book_append_sheet(workbook, worksheet, 'Voice_AI_Agents');
                sheetsCreated++;
                console.log(`[Snapshot Exporter] Created enriched sheet for Voice AI Agents: ${filteredAgents.length} items`);
            }
            // Special handling for AI Employees - enrich with full configuration
            else if (assetType.key === 'ai_employees' && locationId) {
                console.log('[Snapshot Exporter] ✅ AI EMPLOYEES ENRICHMENT TRIGGERED');
                sendProgressUpdate(81, `Enriching ${assets.length} AI employees...`);
                ...
                XLSX.utils.book_append_sheet(workbook, worksheet, 'AI_Employees');
                sheetsCreated++;
                console.log(`[Snapshot Exporter] Created enriched sheet for AI Employees: ${filteredEmployees.length} items`);
            }
            // Special handling for Documents - enrich with template details
            else if (assetType.key === 'documents' && locationId) {
                console.log('[Snapshot Exporter] ✅ DOCUMENTS ENRICHMENT TRIGGERED');
                ...
                XLSX.utils.book_append_sheet(workbook, worksheet, 'Documents');
                sheetsCreated++;
                console.log(`[Snapshot Exporter] Created enriched sheet for Documents: ${filteredDocs.length} items`);
            }
            // Special handling for Snippets - enrich with template details
            else if (assetType.key === 'snippets' && locationId) {
                console.log('[Snapshot Exporter] ✅ SNIPPETS ENRICHMENT TRIGGERED');
                ...
                XLSX.utils.book_append_sheet(workbook, worksheet, 'Snippets');
                sheetsCreated++;
                console.log(`[Snapshot Exporter] Created enriched sheet for Snippets: ${filteredSnippets.length} items`);
            }
...
async function fetchWorkflowTriggers(workflows, locationId) {
    if (!workflows || workflows.length === 0 || !locationId) {
        console.log('[Workflow Triggers] No workflows or missing locationId');
        return [];
    }

    console.log(`[Workflow Triggers] Fetching triggers for ${workflows.length} workflows`);
    const allTriggers = [];
    ...
    try {
        ...
        for (let i = 0; i < workflows.length; i++) {
            ...
            if (!workflowId) {
                console.log(`[Workflow Triggers] [${i + 1}/${workflows.length}] Skipping - no workflow ID`);
                continue;
            }
            ...
            try {
                ...
                console.log(`[Workflow Triggers] [${i + 1}/${workflows.length}] ${workflowName}: ${triggers.length} triggers`);
                ...
                console.log(`[Workflow Triggers] [${i + 1}/${workflows.length}] Error for ${workflowName}: ${error.message}`);
            }
        }

        console.log(`[Workflow Triggers] Total triggers fetched: ${allTriggers.length}`);
    } catch (error) {
        console.error('[Workflow Triggers] Error fetching triggers:', error);
    }
...
async function fetchAllCustomFieldsForLocation(locationId) {
    if (!locationId) {
        console.log('[Custom Field Fetch] Missing locationId');
        return [];
    }

    console.log(`[Custom Field Fetch] Fetching ALL custom fields for location ${locationId}`);
    const allFields = [];
    ...
    try {
        ...
        // First, try to get custom objects to add their IDs to the model types
        try {
            ...
            console.log(`[Custom Field Fetch] Found ${customObjects.length} custom objects`);

            // Add custom object IDs as model types
            customObjects.forEach(obj => {
                const objId = obj.id || obj._id;
                if (objId) {
                    modelTypes.push(objId);
                }
            });
        } catch (error) {
            console.log(`[Custom Field Fetch] Could not fetch custom objects: ${error.message}`);
        }

        console.log(`[Custom Field Fetch] Will fetch custom fields for models: ${modelTypes.join(', ')}`);
        ...
        for (const modelType of modelTypes) {
            try {
                ...
                console.log(`[Custom Field Fetch] Found ${apiFields.length} custom fields for model: ${modelType}`);

                // Add model type to each field if not present
                apiFields.forEach(field => {
                    if (!field.model) {
                ...
                console.log(`[Custom Field Fetch] Error fetching for model ${modelType}: ${error.message}`);
            }
        }

        console.log(`[Custom Field Fetch] Total fields fetched: ${allFields.length}`);
        ...
        console.log('[Custom Field Fetch] Summary by model type:', modelCounts);
        console.log(`[Custom Field Fetch] Total: ${enrichedFields.length} custom fields`);

        return enrichedFields;

    } catch (error) {
        console.error(`[Custom Field Fetch] Error fetching custom field data:`, error);
        return [];
    }
}
        ...
        for (let i = 0; i < knowledgeBases.length; i++) {
            ...
            if (apiData && kbId) {
                ...
                try {
                    ...
                    console.log(`[Knowledge Base Enrichment] [${i + 1}] Fetched ${kbUrls.length} trained URLs`);
                } catch (error) {
                    console.log(`[Knowledge Base Enrichment] [${i + 1}] Could not fetch URLs: ${error.message}`);
                }
                ...
                try {
                    ...
                    console.log(`[Knowledge Base Enrichment] [${i + 1}] Fetched ${kbFaqs.length} FAQs`);
                } catch (error) {
                    console.log(`[Knowledge Base Enrichment] [${i + 1}] Could not fetch FAQs: ${error.message}`);
                }
                ...
                try {
                    ...
                    console.log(`[Knowledge Base Enrichment] [${i + 1}] Fetched ${Array.isArray(kbRichText) ? kbRichText.length : 0} rich text items`);
                } catch (error) {
                    console.log(`[Knowledge Base Enrichment] [${i + 1}] Could not fetch rich text: ${error.message}`);
                }
                ...
                try {
                    ...
                    console.log(`[Knowledge Base Enrichment] [${i + 1}] Fetched operations status`);
                } catch (error) {
                    console.log(`[Knowledge Base Enrichment] [${i + 1}] Could not fetch operations: ${error.message}`);
                }
                ...
                console.log(`[Knowledge Base Enrichment] [${i + 1}] Enriched: ${enrichedKB.totalFiles} files, ${enrichedKB.totalUrls} URLs, ${enrichedKB.totalFaqs} FAQs`);
            } else {
...
async function enrichVoiceAIAgents(agents, locationId) {
    if (!agents || agents.length === 0 || !locationId) {
        console.log('[Voice AI Agent Enrichment] No agents to enrich or missing locationId');
        return agents;
    }

    console.log(`[Voice AI Agent Enrichment] Enriching ${agents.length} Voice AI agents`);
    ...
    try {
        ...
        for (let i = 0; i < agents.length; i++) {
            ...
            console.log(`[Voice AI Agent Enrichment] [${i + 1}/${agents.length}] Fetching details for: ${agentName}`);
            ...
            try {
                ...
                console.log(`[Voice AI Agent Enrichment] [${i + 1}] Enriched: ${enrichedAgent.totalActions} actions, ${enrichedAgent.totalKnowledgeBases} KBs, model: ${enrichedAgent.llmModel}`);
                ...
                console.error(`[Voice AI Agent Enrichment] [${i + 1}] Error fetching agent details:`, error);
        ...
        console.error(`[Voice AI Agent Enrichment] Error in enrichment process:`, error);
        console.log('[Voice AI Agent Enrichment] Returning original data');
        return agents;
    }
...
async function enrichAIEmployees(employees, locationId) {
    if (!employees || employees.length === 0 || !locationId) {
        console.log('[AI Employees Enrichment] No employees to enrich or missing locationId');
        return employees;
    }

    console.log(`[AI Employees Enrichment] Enriching ${employees.length} AI employees`);
    const enrichedEmployees = [];
    ...
    try {
        ...
        for (let i = 0; i < employees.length; i++) {
            ...
            console.log(`[AI Employees Enrichment] [${i + 1}/${employees.length}] Processing: ${employeeName}`);
            ...
            enrichedEmployees.push(enrichedEmployee);
            console.log(`[AI Employees Enrichment] [${i + 1}] Enriched: ${enrichedEmployee.totalActions} actions, mode: ${enrichedEmployee.mode}`);
        }
    } catch (error) {
        console.error(`[AI Employees Enrichment] Error in enrichment process:`, error);
        console.log('[AI Employees Enrichment] Returning original data');
        return employees;
    }
...
async function enrichDocuments(documents, locationId) {
    if (!documents || documents.length === 0 || !locationId) {
        console.log('[Documents Enrichment] No documents to enrich or missing locationId');
        return documents;
    }

    console.log(`[Documents Enrichment] Enriching ${documents.length} documents`);
    const enrichedDocs = [];
    ...
    try {
        ...
        for (let i = 0; i < documents.length; i++) {
            ...
            console.log(`[Documents Enrichment] [${i + 1}/${documents.length}] Processing: ${docName}`);
            ...
            console.log(`[Documents Enrichment] [${i + 1}] Enriched: ${enrichedDoc.type}, items: ${enrichedDoc.totalLineItems}`);
        }
    } catch (error) {
        console.error(`[Documents Enrichment] Error in enrichment process:`, error);
        console.log('[Documents Enrichment] Returning original data');
        return documents;
    }
...
async function enrichSnippets(snippets, locationId) {
    if (!snippets || snippets.length === 0 || !locationId) {
        console.log('[Snippets Enrichment] No snippets to enrich or missing locationId');
        return snippets;
    }

    console.log(`[Snippets Enrichment] Enriching ${snippets.length} snippets`);
    ...
    try {
        for (let i = 0; i < snippets.length; i++) {
            ...
            console.log(`[Snippets Enrichment] [${i + 1}/${snippets.length}] Processing: ${snippetName}`);
        ...
        console.log(`[Snippets Enrichment] Total enriched: ${enrichedSnippets.length} snippets`);
    } catch (error) {
        console.error('[Snippets Enrichment] Error in enrichment process:', error);
        console.log('[Snippets Enrichment] Returning original data');
        return snippets;
    }
